    ...
    <section className="py-24 bg-[var(--card)] border-y border-[var(--border)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-2">
          {top5.map((entry, i) => {
            const kd = entry.player.kills / Math.max(entry.player.deaths, 1);
            const hsRate = (entry.player.headshots / Math.max(entry.player.kills, 1)) * 100;

            return (
              <Link key={entry.player.id} href={`/players/${entry.player.id}`} className="group block">
                <div className={cn(
                  ...
                  <Avatar className={cn("h-10 w-10", i === 0 && "ring-2 ring-yellow-400/50")}>
                    <AvatarImage src={entry.player.avatar} alt={entry.player.nickname} />
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-[var(--muted-foreground)]">
                      {entry.tournaments} torneios · {entry.player.wins}W/{entry.player.losses}L
                    </div>
                  </div>
                  ...
                  <div className="hidden sm:flex items-center gap-6 text-xs text-[var(--muted-foreground)]">
                    <div className="text-center">
                      <div className="font-semibold text-[var(--foreground)]">{kd.toFixed(2)}</div>
                      <div>K/D</div>
                    </div>
                    <div className="text-center">
                      <div className="font-semibold text-[var(--foreground)]">{hsRate.toFixed(0)}%</div>
                      <div>HS</div>
                    </div>
                  </div>
