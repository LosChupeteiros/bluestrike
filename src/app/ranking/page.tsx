import {
  TrendingUp, TrendingDown, Minus, Crown, Medal, Trophy,
  Target, Swords, Users, Search,
} from "lucide-react";
...
export default function RankingPage() {
  const kd = (p: typeof mockRanking[0]["player"]) =>
    (p.kills / Math.max(p.deaths, 1)).toFixed(2);
  const hs = (p: typeof mockRanking[0]["player"]) =>
    ((p.headshots / Math.max(p.kills, 1)) * 100).toFixed(0);
  const wr = (p: typeof mockRanking[0]["player"]) =>
    (((p.wins) / Math.max(p.wins + p.losses, 1)) * 100).toFixed(0);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-3 gap-4 mb-10 max-w-2xl mx-auto">
          {[mockRanking[1], mockRanking[0], mockRanking[2]].map((entry, i) => {
            ...
            return (
              <div key={entry.player.id} className={cn("flex flex-col items-center", i === 1 ? "order-2" : i === 0 ? "order-1" : "order-3")}>
                <Avatar className={cn(
                  "ring-2 mb-2",
                  ...
                  <AvatarFallback>{entry.player.nickname[0]}</AvatarFallback>
                </Avatar>
                <div className="text-xs font-bold text-center truncate w-full px-2">{entry.player.nickname}</div>
                <div className="text-xs text-[var(--primary)] font-black mb-2">{entry.points.toLocaleString()}</div>
                <div className={cn(
            ...
            >
              ...
              <div className="flex items-center gap-3 min-w-0">
                <Avatar className="h-9 w-9 shrink-0">
                  <AvatarImage src={entry.player.avatar} />
                  <AvatarFallback className="text-xs">{entry.player.nickname[0]}</AvatarFallback>
                </Avatar>
                <div className="min-w-0">
                  <div className="font-bold text-sm truncate">{entry.player.nickname}</div>
                  <div className="text-xs text-[var(--muted-foreground)]">
                    {entry.player.wins}W · {entry.tournaments} torneios
                  </div>
                </div>
              </div>

              {/* K/D */}
              <div className="hidden sm:block text-sm font-semibold text-center w-12">{kd(entry.player)}</div>

              {/* HS% */}
              <div className="hidden sm:block text-sm text-center w-12 text-[var(--muted-foreground)]">{hs(entry.player)}%</div>

              {/* Win% */}
              <div className="hidden md:block text-sm text-center w-16">
                <span className="text-green-400 font-semibold">{wr(entry.player)}%</span>
              </div>

              {/* ELO + change */}
              <div className="text-right w-20">
                <div className="text-sm font-black text-[var(--primary)]">{entry.points.toLocaleString()}</div>
                <div className={cn(
                  "flex items-center justify-end gap-0.5 text-xs",
                  entry.change > 0 ? "text-green-400" : entry.change < 0 ? "text-red-400" : "text-[var(--muted-foreground)]"
                )}>
