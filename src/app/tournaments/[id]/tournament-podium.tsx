...
function PodiumColumn({ entry }: { entry: PodiumEntry }) {
  ...
  const content = (
    <>
      ...
      <div className="min-w-0 text-center">
        <div className={`truncate text-sm font-black ${entry.place === 1 ? "text-yellow-300" : entry.place === 2 ? "text-slate-300" : "text-orange-300"}`}>
  ...
  return (
    <div className="flex min-w-0 flex-col items-center gap-3">
      {entry.team ? (
        <Link href={`/teams/${entry.team.slug}`} className="group flex min-w-0 flex-col items-center gap-3">
          {content}
        </Link>
      ) : (
        <div className="flex min-w-0 flex-col items-center gap-3">{content}</div>
      )}
      <div className={`flex ${style.height} w-full flex-col items-center justify-center gap-1 rounded-xl ${style.block}`}>
        <div className="flex items-center gap-1.5">
          <Medal className="h-4 w-4" />
          <span className="text-xs font-black uppercase tracking-wider">{style.label}</span>
        </div>
        {entry.prize > 0 && (
          <span className="text-xs font-black text-[var(--foreground)]">{formatCurrency(entry.prize)}</span>
        )}
      </div>
    </div>
  );
