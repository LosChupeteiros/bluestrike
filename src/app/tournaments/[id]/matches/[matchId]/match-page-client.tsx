...
interface ServerInfo {
  status: string; // "reserving" | "provisioning" | "ready" | "live" | "error"
  rawIp: string | null;
  ip: string;
  port: number;
...
function ServerPanel({
  ...
  async function retryProvision() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) setRetryError(data.error ?? "Erro ao retentar.");
    } catch {
      setRetryError("Erro de rede.");
    } finally {
      setRetrying(false);
    }
  }

  // WhatsApp link for players to call admin
  const matchUrl = `${typeof window !== "undefined" ? window.location.origin : "https://bluestrike.gg"}/tournaments/${tournamentId}/matches/${matchId}`;
  ...
  // ── Error state ──
  if (isError || (server === null && false)) {
    // Error row exists — admin retry + player WhatsApp
  }

  // ── No server yet or error state ──
  if (!server || isError) {
    const isKnownError = isError;
    return (
        ...
        <div className="flex items-center gap-4 px-6 py-6">
          ...
          <div className="flex-1 min-w-0">
            <div className="font-bold text-red-300">
              {isKnownError ? "Falha ao alocar servidor" : "Aguardando servidor…"}
            </div>
            <div className="mt-0.5 text-xs text-[var(--muted-foreground)]">
              {isKnownError
                ? "Ocorreu um erro ao iniciar o servidor CS2."
                : "O servidor CS2 ainda não foi alocado para esta partida."}
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-[var(--border)] px-6 py-4 space-y-3">
          {isAdmin && (
            <button
              ...
              disabled={retrying}
              className="flex w-full items-center justify-center gap-2 rounded-xl border border-[var(--primary)]/30 bg-[var(--primary)]/10 py-3 text-sm font-bold text-[var(--primary)] transition-all hover:bg-[var(--primary)]/15 disabled:opacity-50"
            >
              {retrying ? (
          ...
          {(isPlayer || !isAdmin) && (
            <a
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl border border-green-600/30 bg-green-600/10 py-3 text-sm font-bold text-green-400 transition-all hover:bg-green-600/15"
            >
  ...
  // ── Provisioning — server reserved but still booting ──
  if (!hasConnection) {
    return (
      <div className="overflow-hidden rounded-2xl border border-[var(--primary)]/20 bg-[var(--card)]">
        <div className="flex items-center gap-4 px-6 py-6">
          <div className="relative shrink-0">
            <div className="absolute inset-0 h-10 w-10 animate-ping rounded-full bg-[var(--primary)]/15" />
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-0.5 text-xs text-[var(--muted-foreground)]">
              O CS2 está carregando. O botão de conexão aparece em instantes.
            </div>
          </div>
        ...
        {/* WhatsApp fallback while waiting */}
        {(isPlayer || !isAdmin) && (
          <div className="border-t border-[var(--border)] px-6 py-3">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 rounded-xl border border-green-600/20 bg-green-600/5 py-2.5 text-xs font-semibold text-green-400 transition-all hover:bg-green-600/10"
            >
              <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="currentColor">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
              </svg>
              Problemas? Chamar administrador via WhatsApp
            </a>
          </div>
        )}
      </div>
        ...
        {isPlayer ? (
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-[var(--border)] bg-[var(--secondary)]/60 p-4">
              ...
              <div className="mb-3 cursor-text select-all rounded-lg border border-[var(--border)] bg-black/50 px-4 py-3 font-mono text-sm text-[var(--foreground)] break-all">
                {connectCmd}
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                {server.gotvPort && (
                  <CopyButton value={`connect ${ip}:${server.gotvPort}`} label="GOTV" />
                )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            {server.gotvPort && (
              <div>
                <div className="mb-2 text-[10px] font-bold uppercase tracking-widest text-[var(--muted-foreground)]">
                  GOTV (espectador)
                </div>
                <CopyButton
